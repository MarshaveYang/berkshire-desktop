...
void main() {
  ...
  // 原版这里透明度写死是 1.0（不透明黑底）。改成跟着线条亮度走，
  // 线条以外的区域透明度趋近 0，这样这层画布能叠在已有的渐变背景上面，
  // 而不是整个盖住它。
  float alpha = clamp(max(color.r, max(color.g, color.b)) * 2.6, 0.0, 1.0);
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(0x000000, 0); // 清屏透明，而不是黑色
